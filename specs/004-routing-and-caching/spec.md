...
### User Story 1.5 - 搜尋功能增強與頁面 Title (Priority: P1)

使用者能夠在任何頁面使用全局搜尋功能，搜尋藝人和歌曲，並透過分類篩選查看結果。頁面 title 動態反映當前內容，提升可辨識性和書籤體驗。

**Why this priority**: 搜尋是發現內容的核心功能，全局搜尋提升使用者體驗。動態 title 改善瀏覽器標籤辨識度、書籤管理和 SEO。與 US1 同為 P1，因為這些是基本使用者體驗的組成。

**Independent Test**: 在任何頁面 → 使用 Header 搜尋框輸入關鍵字 → 自動導航到搜尋結果頁 → 觀察瀏覽器標籤 title → 切換分類 → 點擊結果項目 → 確認 title 更新。

**Acceptance Scenarios**:

1. **Given** 使用者在任何頁面，**When** 在 Header 搜尋框輸入關鍵字，**Then** 應用程式即時導航到搜尋結果頁並顯示匹配結果
2. **Given** 使用者在搜尋結果頁，**When** 切換分類（所有/歌曲/藝人），**Then** 顯示對應類別的結果，不重新搜尋
3. **Given** 使用者看到搜尋結果，**When** 點擊歌曲項目，**Then** 導航到歌曲詳情頁
4. **Given** 使用者看到搜尋結果，**When** 點擊藝人卡片，**Then** 導航到藝人詳情頁
5. **Given** 使用者清空搜尋框，**When** 停留在搜尋頁面，**Then** 顯示「輸入藝人或歌曲以搜尋」提示
6. **Given** 使用者在首頁，**When** 觀察瀏覽器標籤，**Then** title 顯示「Music Hits」
7. **Given** 使用者在搜尋頁面，**When** 觀察瀏覽器標籤，**Then** title 顯示「搜尋 | Music Hits」
8. **Given** 使用者在藝人頁面且資料已載入，**When** 觀察瀏覽器標籤，**Then** title 顯示「{藝人名稱} | Music Hits」
9. **Given** 使用者在歌曲頁面且資料已載入，**When** 觀察瀏覽器標籤，**Then** title 顯示「{歌曲名稱} | Music Hits」
...
- **FR-017**: Header 必須包含全局搜尋框，在所有頁面（桌面版）可見，小螢幕隱藏
- **FR-018**: 搜尋框輸入變更時即時更新 URL 並導航到搜尋頁面（使用 `replace: true` 避免污染歷史）
...
- **Search Results（搜尋結果）**: 代表搜尋查詢的結果集，包含唯一藝人列表（去重）和歌曲列表，基於單次 Fuse.js 搜尋執行
- **Search Category（搜尋分類）**: 代表使用者選擇的結果篩選類別（所有/歌曲/藝人），控制搜尋結果的顯示方式
...
- **SC-011**: Header 搜尋框在所有頁面（桌面版）可見，輸入即時導航到搜尋結果頁
...
- **SC-015**: 頁面 title 在資料載入後 100ms 內更新，準確反映內容
...
11. **搜尋範圍**: 假設搜尋僅針對本地 tracks.json 資料，不包含 Spotify API 的即時搜尋
12. **動態 Title**: 假設使用 React hooks 在客戶端更新 document.title，不需要伺服器端渲染支援
...
11. **Spotify API 圖片獲取**: 本次實作使用預設佔位圖，不整合 Spotify API 獲取藝人/歌曲封面
12. **Batch API 優化**: 不實作 getSeveralArtists/getSeveralTracks endpoints 和 providesTags 快取共用策略
13. **進階搜尋功能**: 不包含搜尋歷史、自動完成、搜尋建議、語音搜尋等功能
14. **搜尋結果分頁**: 不實作分頁或無限滾動，顯示所有匹配結果
15. **SEO Meta Tags**: 不包含 Open Graph、Twitter Cards 等社交分享 meta tags
